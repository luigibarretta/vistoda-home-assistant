  ...
  for (const engine of process.env.BROWSER_ENGINE ? [process.env.BROWSER_ENGINE] : ["chromium", "firefox", "webkit"]) {
    ...
    try {
      for (const width of [390, 1280]) {
        ...
        assert.equal(await stage.locator("#rotate").getAttribute("data-rotation"), "auto");
        if (width < 768) assert.match(await stage.locator("#legacy-live").getAttribute("style"), /rotate\(90deg\)/);
        await stage.locator("#rotate").click();
        assert.equal(await stage.locator("#rotate").getAttribute("aria-pressed"), "false");
        assert.doesNotMatch(await stage.locator("#legacy-live").getAttribute("style") || "", /rotate\(90deg\)/);
        await stage.locator("#rotate").click();
