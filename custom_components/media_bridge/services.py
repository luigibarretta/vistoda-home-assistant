...
def async_register(hass: HomeAssistant) -> None:
    ...
    async def handle(call: ServiceCall) -> None:
        ...
        if not isinstance(runtime, BridgeRuntime):
            raise ServiceValidationError("Selected Vistoda Ring bridge is not loaded")
        await async_require_context(hass, call.context, entry.entry_id)
        ...
        if runtime.client is not None:
            ...
            if status is not None and status.online:
                expected_device_id = verified_device_id(entry, status)
                if expected_device_id is None:
                    raise ServiceValidationError(
                        "Ring physical device binding is missing or changed"
                    )
                try:
                    await runtime.client.unlock_ring(alias, expected_device_id=expected_device_id)
                ...
                await runtime.ring_history.async_record("unlock", None, "command:native_service")
                return
        source = resolve_source(hass, OPEN_DOOR, entry)
        ...
        await hass.services.async_call(
            "button", "press", {"entity_id": source}, blocking=True, context=call.context
        )
        hass.bus.async_fire(
            "vistoda_ring_door_open_requested",
            {"path": "official_fallback", "entry_id": entry.entry_id, "alias": alias},
        )
        await runtime.ring_history.async_record("unlock", None, "command:official_service")
