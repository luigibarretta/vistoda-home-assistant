...
class NetworkArchive extends HTMLElement {
  constructor() {
    ...
    this.shadowRoot.innerHTML = `<style>${BASE_STYLES}${PROVIDER_LIST_STYLES}${PROVIDER_BULK_LIST_STYLES}
      ...
      header,.row { justify-content:space-between; } .row { padding:12px 0; border-bottom:1px solid var(--divider-color); }
      .row>div:first-child { min-width:0; overflow-wrap:anywhere; } button { min-width:44px; min-height:44px; }
      code { overflow-wrap:anywhere; } video { width:100%; max-height:60vh; background:#000; }
      select { min-height:44px; max-width:100%; } .muted { font-size:13px; }
      #count { position:sticky; top:0; background:var(--card-background-color); padding:12px 0; z-index:1; }
    </style><header><h3 data-copy="Backup NFS">Backup NFS</h3>
      <button id="reload" aria-label="Aggiorna" data-copy-aria-label="Aggiorna"><ha-icon icon="mdi:refresh"></ha-icon></button></header>
      ...
      <label for="camera" data-copy="Telecamera">Telecamera</label><select id="camera"><option value="" data-copy="Tutte le telecamere">Tutte le telecamere</option></select>
      ${PAGE_SIZE_TEMPLATE}${PROVIDER_LIST_TEMPLATE}${PROVIDER_BULK_LIST_TEMPLATE}
      <div class="actions"><strong id="count"></strong><button id="add-selected-to-lists" disabled
      ...
      <div class="actions"><button id="previous" aria-label="Pagina precedente" data-copy-aria-label="Pagina precedente"><ha-icon icon="mdi:chevron-left"></ha-icon></button>
      <span id="page"></span><button id="next" aria-label="Pagina successiva" data-copy-aria-label="Pagina successiva"><ha-icon icon="mdi:chevron-right"></ha-icon></button></div>`;
